...
# --- настройки -------------------------------------------------------------
say "Пишу $CONF"
# Прежние настройки читаем до перезаписи: повторный запуск не должен молча
# стирать дописанное руками. Адрес монитора попадает сюда именно так, и
# потерять его — значит остаться без единственного сигнала о поломке, не
# заметив этого.
...
# Необязательно: URL внешнего монитора. При успехе пинг уходит на сам адрес,
# при поломке — на <адрес>/fail с причиной, а если машина выключена, не
# уходит ничего и тревогу поднимает сам монитор.
PULL_PING_URL=${PULL_PING_URL:-}
# Необязательно: сообщения в Telegram — об успешной ночи и о поломке.
# Токен бота — у @BotFather, chat_id — id чата, куда бот добавлен.
PULL_TG_TOKEN=${PULL_TG_TOKEN:-}
PULL_TG_CHAT=${PULL_TG_CHAT:-}
CONFEOF
