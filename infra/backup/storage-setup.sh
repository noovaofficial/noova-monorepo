...
# стирать дописанное руками. Токен бота, id чата и список админов попадают
# сюда именно так, и потерять их — значит остаться без сообщений о поломке,
# не заметив этого.
...
# Кому разрешены команды бота (/list, /backup, /verify): числовые user id через
# запятую. Пусто — бот не стартует. Свой id: написать боту @userinfobot.
PULL_TG_ADMINS=${PULL_TG_ADMINS:-}
CONFEOF
