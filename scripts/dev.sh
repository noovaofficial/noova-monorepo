...
#
# То же самое продублировано в скриптах `dev` каждого приложения: запуск
# одного фронта через `pnpm dev:web` идёт мимо этого файла, и без запаса
# наблюдатель упирался в EMFILE после десятка перезапусков.
