...
# пустым, а панель фильтров без услуг. Гоняется каждый выпуск, а не только
# первый, но существующие записи не трогает — только досоздаёт то, чего нет
# (см. предупреждение и разбор в seed-reference.ts): страны, города, районы
# и услуги редактируются из админки (N-32/N-35), и это её область, а не файла.
echo "Справочники…"
