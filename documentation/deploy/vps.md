...
| **SMTP** | почта домена (Stalwart) + лендинг будущего продукта | 22, 25, 80, 443, 465, 993, 8443, 587 (только с Prod) |
...
ssh deploy@<IP релея> 'sudo ufw allow 25,80,443,465,993,8443/tcp'
```

- **587 — только с Prod.** Submission, открытый всему миру, находят за часы.
- **25 — всем.** Сюда указывает MX, написать на домен может любой сервер.
- **465, 993 — всем.** Почтовые клиенты, только TLS.
- **8443 — всем.** Веб-интерфейс Stalwart; 443-й занят лендингом.
- **80, 443 — всем.** Лендинг и ACME-челлендж сертификата почты.
