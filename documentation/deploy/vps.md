...
| **SMTP** | отправка писем + лендинг будущего продукта | 22, 80, 443, 587 (только с Prod) |
...
ssh deploy@<IP релея> 'sudo ufw allow 80,443/tcp'
```

Submission (587) только с Prod: открытый всему миру находят за часы. А 80/443
открыты всем — там лендинг, обычный публичный сайт.

PTR → `mail.<домен>`, в панели провайдера или заявкой. Имя должно совпадать в
трёх местах: HELO, PTR, A-запись `mail.<домен>`. Плюс отдельная A-запись на
голый `<домен>` → тот же IP, для лендинга.
