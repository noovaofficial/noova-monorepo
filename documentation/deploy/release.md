...
> Держите копию `.env` вне сервера. `POSTGRES_PASSWORD` нужен, чтобы открыть
> существующий том базы (дамп встанет и с новым), а `IP_HASH_SALT` —
> чтобы не потерять историю в журналах жалоб и раскрытий.
