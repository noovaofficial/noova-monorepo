...
describe('обработка фотографий', () => {
  ...
  it('клеит вотермарку на каждый вариант ещё до модерации', async () => {
    ...
    // что поверх легли полупрозрачные лого и надпись.
  ...
  it('не растягивает знак вотермарки шире мелкого кадра', async () => {
    // Composite крупнее 100×140 без клампа по ширине и высоте — sharp
    // отказывается накладывать его на изображение меньшего размера.
    const tiny = await sharp({
      create: { width: 100, height: 140, channels: 3, background: '#fff' },
    })
      .jpeg()
