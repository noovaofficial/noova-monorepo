...
export const authRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.post(
    ...
    async (request) => {
      ...
      // Язык — тот, что выбран на сайте: его присылает форма. Заголовок
      // браузера остаётся запасным вариантом и только им: он говорит о
      // настройках браузера, а не о выборе человека, и русскоязычный браузер
      // приносил русские письма даже с немецкого сайта.
      const locale = localeOf(input.locale ?? request.headers['accept-language']?.slice(0, 2));
